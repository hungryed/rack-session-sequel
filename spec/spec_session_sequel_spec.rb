...
describe Rack::Session::SequelMiddleware do
  session_key = Rack::Session::SequelMiddleware::DEFAULT_OPTIONS[:key]
  ...
  it "creates a new cookie" do
    pool = Rack::Session::SequelMiddleware.new(incrementor)
  ...
  it "determines session from a cookie" do
    ...
    expect(req.get("/", "HTTP_COOKIE" => cookie).
      body).to eq '{"counter"=>2}'
    expect(req.get("/", "HTTP_COOKIE" => cookie).
      body).to eq '{"counter"=>3}'
  end
  ...
  it "does not send the same session id if it did not change" do
    ...
    expect(res2["Set-Cookie"]).to eq nil
  ...
  it "does not return a cookie if cookie was not read/written" do
    app = Rack::Session::SequelMiddleware.new(nothing)
    res = Rack::MockRequest.new(app).get("/")
    expect(res["Set-Cookie"]).to eq nil
  end

  it "does not return a cookie if cookie was not written (only read)" do
    app = Rack::Session::SequelMiddleware.new(session_id)
    res = Rack::MockRequest.new(app).get("/")
    expect(res["Set-Cookie"]).to eq nil
  end

  it "returns even if not read/written if :expire_after is set" do
    app = Rack::Session::SequelMiddleware.new(nothing, :expire_after => 3600)
    res = Rack::MockRequest.new(app).get("/", 'rack.session' => {'not' => 'empty'})
    expect(res["Set-Cookie"]).to_not eq nil
  end
